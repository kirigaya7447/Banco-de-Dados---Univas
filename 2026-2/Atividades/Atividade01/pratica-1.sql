...
-- - ALUNO (A) 1: Ana Paula
-- - ALUNO (A) 2: Giulia
-- - ALUNO (A) 3: Joao Henrique
-- - ALUNO (A) 4: Pedro Henrique
...
select sum(qtde) as "Quantidade Total" 
from estoque;
...
select avg(vlr_unitario) as "Preço médio unitário" 
from estoque;
...
select sum(qtde <= 100) as "Quantidade de itens abaixo do mínimo" 
from estoque;

-- QUESTÃO 8
-- Quantos produtos estão acima do estoque mínimo?
select sum(qtde >= 100) as "Quantidade de itens acima do mínimo" 
from estoque;
...
select categoria, sum(qtde > 100) as "Quantidade total" 
from estoque 
group by categoria;
...
select categoria, min(qtde) as "Quantidade mínima", max(qtde) as "Quantidade máxima" 
from estoque 
group by categoria;
